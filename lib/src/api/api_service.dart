...
class BookingApiService {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      data = json.decode(response.body);
      if (data.containsKey('data') && data['data'] is List) {
        results = (data['data'] as List).map((e) {
          var booking = Booking.fromJson(e);
          if (query != null && query.isNotEmpty) {
            var patientFullName = booking.patients?.fullName?.toLowerCase() ?? '';
            var patientPhone = booking.patients?.phoneNumber ?? 0;
            
            if (patientFullName.contains(query.toLowerCase()) || patientPhone.toString().contains(query)) {
              return booking;
            } else {
              return null;
            }
        ...
        }).where((booking) => booking != null).toList().cast<Booking>();
